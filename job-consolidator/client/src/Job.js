...
function getYMD(ts) {
    return ts.toDateString().substring(0, 15);
}

//date function2
function makeDate2(timestamp) {
    const date = new Date(timestamp);
    const dateStr =  getYMD(date);
    return dateStr;
}

function chooseDateFunc(job){
    if(job.created_at) return makeDate(job.created_at);
    else return makeDate2(job.isoDate);
}
...
export default function Job({job, onClick}) {
    ...
    return (
        <Paper onClick={onClick} className='job'>
            ...
            <div className="flex-align-mid">
                <Typography>{ chooseDateFunc(job) }</Typography>
            </div>
        </Paper>
    )
}